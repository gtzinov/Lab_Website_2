...
function loadStatsPage() {
	...
	console.log(myTable.rows[2].cells[1].innerHTML);
	var wins = 0;
	var losses = 0;
	for (i = 2; i < myTable.rows.length; i++) {
		if (myTable.rows[i].cells[2].innerHTML > myTable.rows[i].cells[3].innerHTML) {
			console.log("Test");
			myTable.rows[i].cells[4].innerHTML = "Cu Boulder";
			wins++;

		}

		else {
			myTable.rows[i].cells[4].innerHTML = myTable.rows[i].cells[1].innerHTML;
			losses++;
		}
...
function loadPlayersPage() {
	...
	for (i = 0; i < players.length; i++) {
		...
		anchorElement.href = "#";
		anchorElement.onclick = switchPlayers.bind(null, i);
		anchorElement.innerHTML = players[i].name;
		anchorElement.className = "dropdown-item";

		document.getElementById("player_selector").appendChild(anchorElement);


	}
	// document.getElementById("player_selector").innerHTML = Player1;
